/*
 * tlsh_diff_table.h - auto-extracted from upstream trendmicro/tlsh's
 * src/tlsh_util.cpp (bit_pairs_diff_table), generated there by the
 * upstream repo's gen_arr2.cpp and never hand-edited even upstream -
 * copied verbatim (byte-for-byte, via script, not retyped by hand) here
 * for exactly that reason: this is precomputed pairwise Hamming-ish
 * distance between every (2-bit-quartile-packed) byte pair 0..255, used
 * by h_distance() to score the TLSH body in O(1) per byte instead of
 * unpacking 4 quartile pairs and comparing each - transcription by hand
 * would risk silent single-value errors that only show up as occasional
 * off-by-a-little totalDiff() mismatches, exactly the kind of bug this
 * port's validation pass is designed to catch, so avoiding the risk at
 * the source was cheaper than trying to catch it after the fact.
 * Included only from tlsh_core.c.
 */
#ifndef AV_TLSH_DIFF_TABLE_H
#define AV_TLSH_DIFF_TABLE_H

static const unsigned char bit_pairs_diff_table[256][256] = {
{
