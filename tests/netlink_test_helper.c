...
{
    if (argc < 2) {
        fprintf(stderr, "usage: %s resolve|register|verdict|malformed-verdict|oversized-verdict\n",
                argv[0]);
        return 2;
    }
    ...
    if (!strcmp(argv[1], "verdict")) {
        struct verdict_args a = {0};

        if (argc < 4) {
            fprintf(stderr, "usage: %s verdict <reqid> <0|1> [rule_name]\n", argv[0]);
            return 2;
        }
        a.reqid = strtoull(argv[2], NULL, 10);
        a.verdict = (uint8_t)strtoul(argv[3], NULL, 10);
        a.rule_name = argc > 4 ? argv[4] : NULL;
        return send_and_report(AV_C_VERDICT, fill_verdict, &a);
    }

    if (!strcmp(argv[1], "malformed-verdict")) {
    ...
    if (!strcmp(argv[1], "oversized-verdict")) {
